...
const loadPopup = (() => {
     ...
     */
    const getMatchModeInteractionInfo = (mode, labelText) => ({
        className: "type",
        key: `matchMode.${mode}`,
        label: {
            text: labelText,
     ...
     * @returns The resulting info object.
     */
    const getStorageFieldCheckboxInfo = (storageArea, storageKey) => ({
                    ...
                    interactions: [
                        {
                            ...
                            textbox: {
                                className: "url-input",
                                list: {
                                    getArray: () => storageGet("sync", [StorageSync.URL_FILTERS]).then(sync => //
                                     sync.urlFilters.noPageModify.map(({ hostname, pathname }) => hostname + pathname) //
                                    ),
                                    setArray: array => storageGet("sync", [StorageSync.URL_FILTERS]).then(sync => {
                    ...
                    interactions: [
                        {
                            ...
                            list: {
                                ...
                                removeAt: index => storageGet("sync", [StorageSync.TERM_LISTS]).then(sync => {
                            ...
                            label: {
                                text: "",
                                getText: index => storageGet("sync", [StorageSync.TERM_LISTS]).then(sync => sync.termLists[index] ? sync.termLists[index].name : ""),
                                setText: (text, index) => storageGet("sync", [StorageSync.TERM_LISTS]).then(sync => {
                            ...
                            object: {
                                className: "term",
                                list: {
                                    getArray: index => storageGet("sync", [StorageSync.TERM_LISTS]).then(sync => sync.termLists[index].terms),
                                    setArray: (array, index) => storageGet("sync", [StorageSync.TERM_LISTS]).then(sync => {
