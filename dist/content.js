...
const generateTermHighlightsUnderNode = (() => {
    ...
    const highlightInsideNode = (term, textEndNode, start, end, nodeItems, nodeItemPrevious) => {
        ...
        highlight.parentNode["markmysearchKnown"] = true;
        ...
        const observer = controlsInfo.paintReplaceByClassic
            ...
            for (const mutation of mutations) {
                ...
                if (mutation.type === "characterData"
                    ...
                    boxesInfoCalculateForFlowOwnersFromContent(terms, mutation.target.parentElement, highlightTags, termCountCheck, getHighlightingId, styleUpdates);
                }
                for (const node of Array.from(mutation.addedNodes)) {
                    if (node.nodeType === Node.ELEMENT_NODE) {
                        if (canHighlightElement(rejectSelector, node)) {
                            boxesInfoCalculateForFlowOwnersFromContent(terms, node, highlightTags, termCountCheck, getHighlightingId, styleUpdates);
                        }
                    }
                    else if (node.nodeType === Node.TEXT_NODE
                        && canHighlightElement(rejectSelector, node.parentElement)) {
                        boxesInfoCalculateForFlowOwners(terms, node, highlightTags, termCountCheck, getHighlightingId, styleUpdates);
                    }
                }
            }
        });
...
(() => {
    ...
    const refreshTermControlsAndBeginHighlighting = (() => {
        ...
        return (terms, controlsInfo, commands, highlightTags, hues, mutationUpdates, termCountCheck, produceEffectOnCommand, getHighlightingId, styleUpdates, elementsVisible, termsUpdate, termUpdate, termToUpdateIdx) => {
    ...
    return () => {
        ...
        const requestRefreshIndicators = requestCallFn(() => controlsInfo.paintReplaceByClassic
            ...
            : insertScrollMarkersPaint(terms, hues), () => controlsInfo.paintReplaceByClassic ? 50 : 150, () => controlsInfo.paintReplaceByClassic ? 500 : 2000);
